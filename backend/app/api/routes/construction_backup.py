    ...
    if project.status in ("planning", "active"):
        raise HTTPException(
            ...
            detail=f"Cannot delete a project with status \"{project.status}\". "
                   f"Suspend it first by setting status to \"on_hold\", then delete.",
        )
    log_action(
...
# ── Project child resource GET endpoints (nested RESTful paths) ──────────────
...
def list_contractors(
    ...
    return ContractorService.get_all(db)
...
def update_expense(
    expense_id: int,
    payload: ConstructionExpenseUpdate,
    db: Session = Depends(get_db),
    user: User  = Depends(_admin_manager),
):
    result = ExecutionService.update_expense(db, expense_id, payload)
    log_action(
        db=db, module="construction", action="UPDATE",
        record_id=str(result.id), record_label=f"Expense: {result.description}",
        changed_by=user.email, changed_by_role=getattr(getattr(user, 'role', None), 'name', None),
...
def delete_expense(
    ...
    ExecutionService.delete_expense(db, expense_id)
    log_action(
        db=db, module="construction", action="DELETE",
        record_id=str(expense_id), record_label="Expense deleted",
        changed_by=user.email, changed_by_role=getattr(getattr(user, 'role', None), 'name', None),
    )
    log_activity(
        db=db, user=user, action="delete", module="construction",
        record_type="expense", record_id=expense_id, record_label="Expense deleted",
    )
    db.commit()
