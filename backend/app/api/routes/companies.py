...
def create_company(
    ...
    # ── 2. Admin email uniqueness check (global — email must be unique system-wide
    #       for the login flow to work unambiguously) ──────────────────────────
    if db.query(User).filter(User.email == payload.admin_user.email).first():
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        _seed_features(db, company.id)

        # ── 5. Seed company-scoped roles (Admin / Manager / Staff) ───────────
        #       We create company-specific copies so permissions can be tuned
        #       per-tenant without affecting other companies.
        admin_role = _seed_company_roles(db, company.id)
        ...
        log_create(
            ...
            description=f"Company '{company.name}' created with admin user '{new_admin.email}'",
            details={"slug": company.slug, "plan": company.plan, "admin_email": new_admin.email},
            request=request,
        )
...
def create_company_admin(
    ...
    if not admin_role:
        admin_role = _seed_company_roles(db, company_id)
