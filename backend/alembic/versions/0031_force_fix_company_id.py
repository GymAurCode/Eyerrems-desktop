...
Revision ID: 0031_force_fix_company_id
Revises: 0030_dummy_alt
...
revision = "0031_force_fix_company_id"
down_revision = "0030_dummy_alt"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    for table in ALL_TABLES:
        ...
        if "company_id" not in columns:
            print(f"[0031] Adding missing company_id column to table: {table}")
            op.add_column(
                table,
        ...
        if table in ["invoices", "commissions"] and "description" not in columns:
            print(f"[0031] Adding missing description column to table: {table}")
            op.add_column(
                table,
