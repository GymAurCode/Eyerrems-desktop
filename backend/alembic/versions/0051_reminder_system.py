...
def upgrade() -> None:
    ...
    if not _table_exists("reminder_templates"):
        op.create_table(
            "reminder_templates",
        ...
        existing_indexes = {idx["name"] for idx in inspect(conn).get_indexes("reminder_templates")}
        if "ix_reminder_templates_id" not in existing_indexes:
            op.create_index(op.f("ix_reminder_templates_id"), "reminder_templates", ["id"])
        if "ix_reminder_templates_user_id" not in existing_indexes:
    ...
    if not _table_exists("reminders"):
        ...
        existing_indexes = {idx["name"] for idx in inspect(conn).get_indexes("reminders")}
        if "ix_reminders_id" not in existing_indexes:
            op.create_index(op.f("ix_reminders_id"), "reminders", ["id"])
        if "ix_reminders_user_id" not in existing_indexes:
            op.create_index(op.f("ix_reminders_user_id"), "reminders", ["user_id"])
        if "ix_reminders_category" not in existing_indexes:
            op.create_index(op.f("ix_reminders_category"), "reminders", ["category"])
        if "ix_reminders_remind_at" not in existing_indexes:
    ...
    if not _table_exists("notification_logs"):
        ...
        existing_indexes = {idx["name"] for idx in inspect(conn).get_indexes("notification_logs")}
        if "ix_notification_logs_id" not in existing_indexes:
            op.create_index(op.f("ix_notification_logs_id"), "notification_logs", ["id"])
        if "ix_notification_logs_reminder_id" not in existing_indexes:
