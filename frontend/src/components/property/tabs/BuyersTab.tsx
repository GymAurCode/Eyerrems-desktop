      ...
      <div className="card-dark overflow-hidden" style={{ border: "1px solid var(--border)" }}>
        {buyers.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-secondary text-sm">No buyers yet.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {buyers.map((b) => (
                <tr key={b.id} className="transition-colors row-hover"
                  style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                  <td className="px-4 py-3 font-mono text-xs text-blue-400">{b.tid}</td>
                  <td className="px-4 py-3 text-primary font-medium">{b.name}</td>
                  <td className="px-4 py-3 text-secondary">{b.email || "—"}</td>
                  <td className="px-4 py-3 text-secondary">{b.phone || "—"}</td>
                  <td className="px-4 py-3 text-secondary">{b.address || "—"}</td>
                  <ActionsCell>
                    <QuickRowActions row={b} compact onPrint={(row) => printRecord(`Buyer ${row.tid}`, [
