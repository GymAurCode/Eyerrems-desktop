...
export default function AddBuyerDialog({ isOpen, onClose, onSaved, editContact }: AddBuyerDialogProps) {
  ...
  const resetDialog = () => {
    setName(""); setRole(["buyer"]); setContactType("individual");
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await propApi.updateContact(editing.id, payload);
  ...
  const handleMergeDuplicate = async () => {
    ...
    const newRoles = [...role];
    const addRole = role.includes("seller") ? "seller" : "buyer";
    if (!newRoles.includes(addRole)) newRoles.push(addRole);
    try {
      await propApi.addContactRole(duplicateWarn.id, addRole);
      await propApi.updateContact(duplicateWarn.id, { role: newRoles.join(",") });
      setDuplicateWarn(null);
      resetDialog();
      onClose();
      onSaved();
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="space-y-6">
        ...
        <FormSection title="Financial Information (KYC)">
