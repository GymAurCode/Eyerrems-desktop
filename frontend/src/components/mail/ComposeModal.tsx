...
import { X, Paperclip, Send, Bold, Italic, Underline, List, AlignLeft, Minus, Mail } from "lucide-react";
...
export default function ComposeModal({ accounts, defaultAccountId, replyTo, onClose, onSent }: Props) {
  ...
  return (
    <AppDialog isOpen={true} title={replyTo ? "Reply" : "New Message"} subtitle="Compose email" size="lg" icon={<Mail size={16} />} onClose={onClose}
      footer={
      ...
      <div className="flex flex-col gap-0">
        ...
        <div className="flex items-center gap-2 py-2">
          ...
          <button
            ...
            className="text-[10px] text-muted hover:text-primary transition-colors shrink-0"
          >
            CC/BCC
          </button>
        </div>
        ...
        <div
          ...
          className="flex-1 min-h-[200px] py-3 text-sm text-primary focus:outline-none"
        ...
        {attachments.length > 0 && (
          <div className="flex flex-wrap gap-2 py-2 border-t border-theme">
            {attachments.map((a) => (
              <div
                key={a.id}
