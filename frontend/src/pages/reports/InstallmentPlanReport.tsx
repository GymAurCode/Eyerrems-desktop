/**
 * InstallmentPlanReport — Enterprise Installment Plan Report page.
 *
 * Flow:
 *   1. User searches for a booking (by booking ID string or client name)
 *   2. System loads full booking + client + property + installment data
 *   3. Live A4-style preview renders in the browser
...
import { InstallmentPlanPreview } from "../../components/reports/EnterpriseDocumentPreview";
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function InstallmentPlanReport() {
  ...
  const loadReport = useCallback(async (booking: BookingListItem) => {
    ...
    setError(null);
    try {
      const data = await generateReport("installment_plan", {
  ...
  const handleExportPDF = async () => {
    ...
    setExporting(true);
    try {
      const blob = await exportReport("installment_plan", {
        booking_id: selectedBooking.id,
        format: "pdf",
        export_mode: true,
      });
      downloadBlob(blob, `installment_plan_${selectedBooking.booking_id}.pdf`);
    } catch (err: any) {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      downloadBlob(blob, `installment_plan_${selectedBooking.booking_id}.xlsx`);
    } catch (err: any) {
  ...
  // Inject meta into docData for the preview
    ...
    <div className="flex flex-col h-full min-h-0 overflow-hidden" style={{ background: "var(--bg-base)" }}>
      ...
      <div className="shrink-0 px-5 py-3 border-b flex items-center gap-3"
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div>
            <p className="text-sm font-bold text-primary leading-none">Installment Plan Report</p>
            <p className="text-[10px] text-muted mt-0.5 hidden sm:block">
              Enterprise financial booking report with full installment schedule
            </p>
          </div>
        </div>
        ...
        <div className="w-full px-5 py-5 space-y-5">
            ...
            <div className="rounded-xl border p-5 space-y-4"
              ...
              <div>
                ...
                <p className="text-xs text-muted mt-0.5">
                  Search for a booking to generate the installment plan report
                </p>
              </div>
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-sm text-muted">Loading installment plan...</p>
            </div>
          )}
